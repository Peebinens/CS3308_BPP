...
class PackingEnv(gym.Env):
    ...
    def next_box(self) -> list:
        _next_box = self.box_creator.preview(1)[0]
        if len(_next_box) == 3:
            self.box_name = None
        else:
            self.box_name = _next_box[3]
        return _next_box[0:3]
    ...
    def step(self, action):
        ...
        if self.box_name is not None:
            if rot == 1:
                print(f"        name: {self.box_name['sku_code']} \tdim: {self.box_name['dimensions']['width']}, {self.box_name['dimensions']['length']}, {self.box_name['dimensions']['height']} \tpos: {pos[0]}, {pos[1]}, {pos[2]}")
            else:
                print(f"        name: {self.box_name['sku_code']} \tdim: {self.box_name['dimensions']['length']}, {self.box_name['dimensions']['width']}, {self.box_name['dimensions']['height']} \tpos: {pos[0]}, {pos[1]}, {pos[2]}")

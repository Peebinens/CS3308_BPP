...
if __name__ == '__main__':
    ...
    threads_dataset = split_array_fixed_size(rotate_dataset, len(rotate_dataset) // 8 + 1)
    import threading
    threads = []
    for thread_dataset in threads_dataset:
        t = threading.Thread(target=one_thread, args=(args, thread_dataset))
        threads.append(t)
        t.start()
    for t in threads:
        t.join()

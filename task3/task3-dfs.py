...
def generate_swapped_combinations(data):
    ...
    for sublist in _data:
        ...
        #     swap_options.append([[first, second, third]])
        # elif first == second:
        #     swap_options.append([
        ...
        # elif second == third:
        #     swap_options.append([
        #         [second, third, first],  # 与第二个值交换
        #         [first, second, third],  # 保持不变
        #     ])
        # elif first == third:
        #     swap_options.append([
        #         [third, third, second],  # 与第一个值交换
        #         [first, second, third],  # 保持不变
        #     ])
        # else :
        #     swap_options.append([
...
def test_dfs(args):
    ...
    rotate_dataset = generate_swapped_combinations(args.env.box_size_set)
    true_rotate_dataset = []
    for item in rotate_dataset:
        true_rotate_dataset.append(item[::-1])
    # with open("rotate_dataset.txt", "w") as f:
    #     for item in true_rotate_dataset:
    #         f.write(str(item) + '\n')
    ...
    best_list = []
    now_cnt = 0
    for item_list in true_rotate_dataset:
        now_cnt += 1
        if cut_branch(item_list, del_set):
            # print(f"cut: {item_list}")
            continue
        ...
        if ok_ratio > best_ratio:
            ...
            best_list = item_list
            best_num = ok_num
    return best_num, best_ratio, best_list
if __name__ == '__main__':
    ...
    for sta_code, sku_list in test_dataset:
        ...
        while len(box_size_set) > 0:
            ...
            args.seed = 80
            args.test_episode = 1
            ok_num, ok_ratio, ok_list = test_dfs(args)
